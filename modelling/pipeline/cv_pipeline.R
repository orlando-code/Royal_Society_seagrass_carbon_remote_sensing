...
load_packages(c("here", "mgcv", "tidyverse", "ggplot2", "randomForest", "blockCV",
...
## ================================ AUTOCORRELATION (COMMENTED OUT) ================================
## Using only random split + predefined spatial block sizes (1, 5, 25, 50, 100 km).
# autocor_summary <- NULL
# autocor_csv <- "data/VRE/autocorrelation_summary.csv"
# autocor_rds <- "data/VRE/autocorrelation_results.rds"
# if (file.exists(autocor_csv)) { ... } else if (file.exists(autocor_rds)) { ... }
...
# 2) Environmental clustering (commented out – using only random split + spatial blocks)
# X_env <- scale(as.matrix(core_data_complete[, predictor_vars]))
# env_km <- kmeans(X_env, centers = n_folds, nstart = 25)
# cv_strategies[[length(cv_strategies) + 1]] <- list(method = "env_cluster", folds = env_km$cluster, block_size_m = NA)

# 3) Distance-buffered CV (commented out – using only random split + spatial blocks)
# dist_buffered_folds <- create_distance_buffered_folds(core_sf, n_folds, buffer_m = buffer_m)
# cv_strategies[[length(cv_strategies) + 1]] <- list(method = paste0("distance_buffer_", buffer_km, "km"), folds = dist_buffered_folds, block_size_m = NA, buffer_m = buffer_m)
...
cat("========================================\n\n")
# run_inla_in_cv <- TRUE # TODO: remove this
# if (!run_inla_in_cv) cat("(INLA skipped in CV; set run_inla_in_cv <- TRUE to include it.)\n\n")

# Run CV for each strategy; predictor sets:
# - 'all' (always)
# - optional pruned sets (GAM / XGBoost / GPR) when available and
#   use_permutation_predictor_sets == TRUE. These pruned sets are typically
#   derived using correlation + permutation-importance-based pruning scripts
#   (e.g. covariate_pruning.R, gpr_covariate_pruning.R), so this flag lets you
#   turn them on/off from a single place.
...
for (i in seq_along(cv_strategies)) {
  ...
  for (ps_name in names(predictor_sets)) {
    ...
    run_idx <- run_idx + 1
    cat("  [", run_idx, "/", n_runs, "] ", s$method, ", predictor_set = ", ps_name, " ...\n", sep = "")
    res <- tryCatch({
      run_cv(
...
# ## ================================ SENSITIVITY: WITHOUT HIGH-RANGE VARIABLES ================================
# results_df$predictor_set <- "all"
# if (exists("quick_cv_check") && isTRUE(quick_cv_check)) {
#   cat("Quick check: skipping sensitivity (reduced predictors).\n\n")
# } else if (length(high_range_vars) > 0) {
#   drop_vars <- character(0)
#   for (v in high_range_vars) {
#     if (v %in% names(autocor_to_predictor)) {
#       drop_vars <- c(drop_vars, autocor_to_predictor[[v]])
#     } else if (v %in% predictor_vars) {
#       drop_vars <- c(drop_vars, v)
#     }
#   }
#   drop_vars <- unique(drop_vars)
#   drop_vars <- drop_vars[drop_vars %in% predictor_vars]
#   if (length(drop_vars) > 0) {
#     predictor_vars_reduced <- setdiff(predictor_vars, drop_vars)
#     if (length(predictor_vars_reduced) < 2) {
#       cat("Sensitivity skipped: would leave < 2 predictors.\n\n")
#     } else {
#       core_reduced <- dat %>%
#       dplyr::select(dplyr::all_of(predictor_vars_reduced), median_carbon_density, longitude, latitude) %>%
#       filter(complete.cases(.))
#     cat("\n--- Sensitivity: CV without high-range predictors ---\n")
#     cat("Excluded:", paste(drop_vars, collapse = ", "), "\n\n")
#     reduced_results <- list()
#     for (i in seq_along(cv_strategies)) {
#       s <- cv_strategies[[i]]
#       buf_m <- if (!is.null(s$buffer_m)) s$buffer_m else NA_real_
#       res_red <- tryCatch({
#         run_cv(s$method, s$folds, core_reduced, predictor_vars_reduced,
#                tune_hyperparams = use_hyperparameter_tuning,
#                nested_tuning = nested_tuning, verbose = FALSE,
#                buffer_m = buf_m, core_sf = core_sf)
#       }, error = function(e) NULL)
#       if (!is.null(res_red) && nrow(res_red) > 0) {
#         res_red$block_size_m <- s$block_size_m
#         res_red$predictor_set <- "without_high_range"
#         reduced_results[[length(reduced_results) + 1]] <- res_red
#       }
#     }
#     if (length(reduced_results) > 0) {
#       results_df <- dplyr::bind_rows(results_df, dplyr::bind_rows(reduced_results))
#     }
#     }
#   }
# }
...
# Colours: grouped by method type for better distinguishability
# - Two shades of blue for kriging (OK, UK)
# - Two shades of red for GAM/INLA
# - Two shades of green for XGBoost/RF
# - Two shades of purple for NN/SVM
# - Orange/yellow for GPR
model_colours <- c(
...
ggsave(file.path(out_dir, "cv_metrics_by_method_model.png"), p_metrics, width = 12, height = 8, dpi = 150)
...
if (!is.null(predictions_df) && nrow(predictions_df) > 0) {
  ...
  if (nrow(pred_all) > 0) {
    ...
    ggsave(file.path(out_dir, "cv_observed_vs_predicted.png"), p_scatter, width = 12, height = 10, dpi = 150)
...
ggsave(file.path(out_dir, "cv_ensemble_by_method.png"), p_method, width = 10, height = 5, dpi = 150)
...
if ("predictor_set" %in% names(results_df) && "without_high_range" %in% results_df$predictor_set) {
  ...
  p_exclude_r2 <- ggplot(sum_exclude, aes(x = predictor_set, y = mean_r2, fill = predictor_set)) +
    ...
    labs(x = "Predictor set", y = "Mean R² (± SD)", title = "Effect of excluding high-autocorrelation variables") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  p_exclude_rmse <- ggplot(sum_exclude, aes(x = predictor_set, y = mean_rmse, fill = predictor_set)) +
    ...
    facet_wrap(~model, ncol = 3) +
    labs(x = "Predictor set", y = "Mean RMSE (± SD)", title = "Effect of excluding high-autocorrelation variables") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
  p_exclude_combined <- p_exclude_r2 + p_exclude_rmse + plot_layout(ncol = 1)
  ggsave(file.path(out_dir, "cv_effect_excluding_high_autocorrelation.png"), p_exclude_combined, width = 10, height = 8, dpi = 150)
...
if (nrow(spatial_results) > 0 && "block_size_m" %in% names(spatial_results)) {
  ...
  ggsave(file.path(out_dir, "cv_performance_over_block_sizes.png"), p_block_combined, width = 9, height = 7, dpi = 150)
...
if (nrow(summary_unc) > 0) {
  ...
  ggsave(file.path(out_dir, "cv_uncertainty_capable_models.png"), p_unc_simple, width = 8, height = 5, dpi = 150)
...
for (idx in seq_along(cv_strategies)) {
  ...
  if (is.null(blocks_sf)) {
    cv_out <- tryCatch({
      blockCV::cv_spatial(x = core_sf, k = n_folds, size = s$block_size_m, selection = "random",
...
if (length(block_illustration_data) > 0) {
  ...
  p_blocks_illustration <- patchwork::wrap_plots(plot_list, ncol = 2, byrow = TRUE) +
    plot_annotation(
      title = "Spatial blocks (1, 5, 25, 50, 100 km) superposed on data points (fold 1 = test)",
  ...
  ggsave(file.path(out_dir, "cv_spatial_blocks_and_points.png"), p_blocks_illustration,
         width = 10, height = max(5, 2.5 * ceiling(n_panels / 2)), dpi = 150)
...
cat("CV strategies: Random split + spatial block at 1, 5, 25, 50, 100 km.\n")
...
cat("\nPlots: cv_metrics_by_method_model.png, cv_observed_vs_predicted.png, cv_gam_spatial_predictions_and_errors.png,\n")
cat("cv_gam_vs_ml_by_method.png, cv_random_vs_spatial_gam_ml.png, cv_performance_over_block_sizes.png,\n")

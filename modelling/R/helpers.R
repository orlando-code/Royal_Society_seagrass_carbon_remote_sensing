...
get_pruned_predictors_for_model <- function(model_name,
  ...
  if (is.null(csv_path)) return(NULL)
  df <- tryCatch(
    read.csv(csv_path, stringsAsFactors = FALSE),
    error = function(e) NULL
  )
  if (is.null(df) || !all(c("model", "variable", "keep") %in% names(df))) return(NULL)
...
calculate_core_aggregate <- function(core_data, aggregation_level = 5, aggregation_function = median) {
  core_data %>%
    ...
    summarise(aggregate_carbon_density = aggregation_function(carbon_density_g_c_cm3, na.rm = TRUE),
              .groups = "drop")
}
...
fit_ok <- function(train_data, test_data, predictor_vars = NULL, nmax = 25) {
  ...
  train_agg <- train_data %>%
    ...
    as.data.frame()
  if (nrow(train_agg) < 4) return(list(model = NULL, predictions = rep(NA_real_, nrow(test_data))))
  ...
  if (is.null(v) || nrow(v) < 3) return(list(model = NULL, predictions = rep(NA_real_, nrow(test_data))))
  ...
  if (is.null(pred)) return(list(model = list(variogram = fit), predictions = rep(NA_real_, nrow(test_data))))
...
fit_uk <- function(train_data, test_data, predictor_vars, nmax = 25) {
  ...
  if (length(pv) == 0) return(list(model = NULL, predictions = rep(NA_real_, nrow(test_data))))
...
fit_inla_cv <- function(train_data, test_data, predictor_vars) {
  ...
  if (inherits(bnd, "try-error")) return(list(model = NULL, predictions = rep(NA_real_, nrow(test_data))))
  mesh <- try(INLA::inla.mesh.2d(boundary = bnd, max.edge = c(0.5, 2), cutoff = 0.2), silent = TRUE)
  if (inherits(mesh, "try-error")) return(list(model = NULL, predictions = rep(NA_real_, nrow(test_data))))
  ...
  if (inherits(fit, "try-error")) return(list(model = NULL, predictions = rep(NA_real_, nrow(test_data))))
...
create_distance_buffered_folds <- function(core_sf, n_folds, buffer_m = 1000) {
  ...
  for (i in remaining) {
    d <- sapply(seq_len(n_folds), function(k) {
      if (length(folds[[k]]) == 0) return(Inf)
...
compare_depth_profile_models <- function(dat_clean, 
  ...
  if (!is.null(models$exp_null)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "exp_null", aic = aic_val, bic = bic_val, 
  ...
  if (n_successful > 0) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "exp_species", aic = aic_total, bic = bic_total,
  ...
  if (!is.null(models$power_null)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "power_null", aic = aic_val, bic = bic_val,
                 n_params = 2, has_species_interaction = FALSE))
  }
  ...
  if (n_successful > 0) {
    ...
    bic_total <- aic_total + n_params_total * log(n_obs)
    model_comparison <- rbind(model_comparison,
      data.frame(model = "power_species", aic = aic_total, bic = bic_total,
                 n_params = n_params_total, has_species_interaction = TRUE))
  }
  ...
  if (!is.null(models$linear_null)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "linear_null", aic = aic_val, bic = bic_val,
                 n_params = 2, has_species_interaction = FALSE))
  }
  ...
  if (!is.null(models$linear_species)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "linear_species", aic = aic_val, bic = bic_val,
  ...
  if (!is.null(models$gam_null)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "gam_null", aic = aic_val, bic = bic_val,
  ...
  if (!is.null(models$gam_species_by)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "gam_species_by", aic = aic_val, bic = bic_val,
                 n_params = n_params, has_species_interaction = TRUE))
  }
  ...
  if (!is.null(models$gam_tensor)) {
    ...
    model_comparison <- rbind(model_comparison,
      data.frame(model = "gam_tensor", aic = aic_val, bic = bic_val,
                 n_params = n_params, has_species_interaction = TRUE))
  }
...
investigate_depth_profile_models <- function(dat_clean,
  ...
  if (nrow(results_norm$comparison) > 0) {
    ...
    if (nrow(best_with_interaction) > 0 && nrow(best_without_interaction) > 0) {
      ...
      cat("   Best model without species interaction:", best_without_interaction$model, 
          "(AIC =", round(best_without_interaction$aic, 2), ")\n")
      cat("   Best model with species interaction:", best_with_interaction$model,
...
run_cv <- function(cv_method_name, fold_indices, core_data, predictor_vars, tune_hyperparams = FALSE, nested_tuning = TRUE, verbose = FALSE, buffer_m = NA_real_, core_sf = NULL, return_predictions = FALSE, skip_inla = FALSE, models = NULL) {
  all_models <- c("Random Forest", "XGBoost", "SVM", "Neural Network", "GPR", "Ordinary Kriging", "Universal Kriging (env drift)", "GAM", "INLA")
  ...
  if(tune_hyperparams && !nested_tuning) {
    ...
    if ("Random Forest" %in% models) rf_params <- tune_hyperparameters("rf", first_train_data, predictor_vars, verbose = verbose)
    if ("XGBoost" %in% models) xgb_params <- tune_hyperparameters("xgb", first_train_data, predictor_vars, verbose = verbose)
    if ("Neural Network" %in% models) nn_params <- tune_hyperparameters("nn", first_train_data, predictor_vars, verbose = verbose)
  ...
  for(fold in 1:num_folds) {
    ...
    if(tune_hyperparams && nested_tuning) {
      ...
      if ("Random Forest" %in% models) rf_params <- tune_hyperparameters("rf", train_data, predictor_vars, verbose = verbose)
      if ("XGBoost" %in% models) xgb_params <- tune_hyperparameters("xgb", train_data, predictor_vars, verbose = verbose)
      if ("Neural Network" %in% models) nn_params <- tune_hyperparameters("nn", train_data, predictor_vars, verbose = verbose)
    }
    ...
    if ("Random Forest" %in% models) tryCatch({
      ...
      if (return_predictions) preds_list[[length(preds_list) + 1]] <- data.frame(method = cv_method_name, fold = fold, model = "Random Forest", observed = test_data$median_carbon_density, predicted = rf_result$predictions, longitude = test_data$longitude, latitude = test_data$latitude, stringsAsFactors = FALSE)
    }, error = function(e) {
      cat("    RF fold", fold, ":", e$message, "\n")
    })
    if ("XGBoost" %in% models) tryCatch({
      ...
      if (return_predictions) preds_list[[length(preds_list) + 1]] <- data.frame(method = cv_method_name, fold = fold, model = "XGBoost", observed = test_data$median_carbon_density, predicted = xgb_result$predictions, longitude = test_data$longitude, latitude = test_data$latitude, stringsAsFactors = FALSE)
    }, error = function(e) {
      cat("    XGB fold", fold, ":", e$message, "\n")
    })
    if ("SVM" %in% models) tryCatch({
    ...
    if ("Neural Network" %in% models) tryCatch({
      ...
      if (return_predictions) preds_list[[length(preds_list) + 1]] <- data.frame(method = cv_method_name, fold = fold, model = "Neural Network", observed = test_data$median_carbon_density, predicted = nn_result$predictions, longitude = test_data$longitude, latitude = test_data$latitude, stringsAsFactors = FALSE)
    }, error = function(e) {
      cat("    NN fold", fold, ":", e$message, "\n")
    })
    if ("GPR" %in% models) tryCatch({
    ...
    if ("Ordinary Kriging" %in% models) tryCatch({
      ...
      if (return_predictions && !all(is.na(ok_result$predictions))) preds_list[[length(preds_list) + 1]] <- data.frame(method = cv_method_name, fold = fold, model = "Ordinary Kriging", observed = test_data$median_carbon_density, predicted = ok_result$predictions, longitude = test_data$longitude, latitude = test_data$latitude, stringsAsFactors = FALSE)
    }, error = function(e) {
      cat("    OK fold", fold, ":", e$message, "\n")
      results_list[[paste0("fold_", fold, "_ok")]] <<- data.frame(
        method = cv_method_name, fold = fold, model = "Ordinary Kriging",
        r2 = NA_real_, rmse = NA_real_, mae = NA_real_, bias = NA_real_
      )
    })
    if ("Universal Kriging (env drift)" %in% models) tryCatch({
      ...
      results_list[[paste0("fold_", fold, "_uk")]] <- data.frame(
        method = cv_method_name,
        fold = fold,
        model = "Universal Kriging (env drift)",
        uk_metrics
      )
      if (return_predictions && !all(is.na(uk_result$predictions))) preds_list[[length(preds_list) + 1]] <- data.frame(method = cv_method_name, fold = fold, model = "Universal Kriging (env drift)", observed = test_data$median_carbon_density, predicted = uk_result$predictions, longitude = test_data$longitude, latitude = test_data$latitude, stringsAsFactors = FALSE)
    }, error = function(e) {
    ...
    if ("INLA" %in% models && !skip_inla) tryCatch({
...
plot_model_comparison <- function(comparison_results) {
  ...
  comp_long <- comp %>%
    ...
    pivot_longer(cols = c(delta_aic, delta_bic), 
                 names_to = "criterion", 
                 values_to = "delta") %>%
    mutate(criterion = ifelse(criterion == "delta_aic", "Delta AIC", "Delta BIC"))
  
  p <- ggplot(comp_long, aes(x = reorder(model, delta), y = delta, 
    ...
    scale_fill_manual(values = c("FALSE" = "lightblue", "TRUE" = "lightcoral"),
                      name = "Species\ninteraction") +
    labs(x = "Model", y = "Delta (relative to best model)",
...
plot_raster_stack <- function(raster_stack, 
  ...
  for (i in seq_along(layer_names)) {
    ...
    p <- p +
      ...
      p <- p + coord_cartesian(xlim = c(min(layer_data$x), max(layer_data$x)),
